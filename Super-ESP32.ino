...
//   /____/\__,_/ .___/\___/_/        /_____//____/_/          //
//             /_/                                             //
//   ------------------ By Rik Heijmann --------------------   //
//                                                             //
/////////////////////////////////////////////////////////////////
...
// Enable or disable the website/api module. If not used disable it. This allows the firmware file to be smaller and to use less resources.
#define WEBSITE 1
// HTTPS using ESPAsyncWebserver is currently not supported on the ESP32.
#define WEBSITE_SSL 0
#define WEBSITE_PORT 80

// Pins that are automatically configured as on/off switches on the website and on MQTT.
int gpio_outputs[] =          {2,             14,        15,        27       }; // Signifies each GPIO pin that is going to be turned on and off using the API and website.
String gpio_outputs_names[] = {"Onboard LED", "GPIO 14", "GPIO 15", "GPIO 27"}; // Adds to the '2D' 'gpio_outpus' matrix. Signifies the label/name of each pin.
int gpio_outputs_defaults[] = {1,             0,         0,         0        }; // Adds to the '2D' 'gpio_outpus' matrix. Signifies the default power mode on boot of each pin. 1 to turn on during boot, 0 to do the opposite.
...
///////////////////////
// Required for webserver management
#if WEBSITE == true
  // HTTPS using ESPAsyncWebserver is currently not supported on the ESP32.
  //#if WEBSITE_SSL == true
  ...
  #include <ESPAsyncWebServer.h> // Webserver library. Integrates with ESPAsyncTCP or AsyncTCP. Available from: https://github.com/me-no-dev/ESPAsyncWebServer
  #ifdef ESP8266
    #include <ESPAsyncTCP.h> // Available from: https://github.com/me-no-dev/AsyncTCP
  #endif
  #ifdef ESP32
    #include <AsyncTCP.h> // Available from: https://github.com/me-no-dev/AsyncTCP
  #endif
...
typedef struct { // Defines all possible dynamic configuration options. These can be changed using the API.
  ...
  char mqtt_topics[128];
#endif

#if WEBSITE == true
  char website_username[32];
  char website_password[64];
#endif
} Configuration;
...
#if WEBSITE == true
  AsyncWebServer website(WEBSITE_PORT); // Creates a "webserver" object. Please see the "WEBSITE_PORT" directive to change the port number. This number has been integrated in different parts of the code. Note: It can't be dynamically changed.
#endif
      ...
      if (configuration_file_temp_size <= configuration_file_size) { // The size of the configuration file has to be predefined. The configuration file in SPIFFS should match this predefined setting.
        ...
        if (!deserializeJson(configuration_json, configuration_file)) { // Fill the configuration JSON structure with the contents of the configuration file.
          ...
          if (configuration_json.containsKey("mqtt")) {
            ...
            if (configuration_json_mqtt.containsKey("topics")) {
              strcpy(configuration.mqtt_topics, configuration_json_mqtt["topics"]);
            } else {
              Serial.println("[✗] Failed to find required configuration option 'mqtt.topics'.");
              return false;
            }
          ...
          if (configuration_json.containsKey("website")) {
            JsonObject configuration_json_website = configuration_json["website"];

            if (configuration_json_website.containsKey("username")) {
              strcpy(configuration.website_username, configuration_json_website["username"]);
            } else {
              Serial.println("[✗] Failed to find configuration option 'website.username'. Falling back to the default username.");
              strcpy(configuration.website_username, "admin");
            }

            if (configuration_json_website.containsKey("password")) {
              strcpy(configuration.website_password, configuration_json_website["password"]);
            } else {
              Serial.println("[✗] Failed to find configuration option 'website.password'. Falling back to the default username.");
              strcpy(configuration.website_password, "P@$$w0RD!");
            }
          } else {
            Serial.println("[✗] The required 'website' configuration section could not be found.");
            return false;
          }
#endif
...
bool saveConfiguration(const char *filename, const Configuration &configuration) {
  ...
  if (SPIFFS.begin()) { // Checks if the SPIFFS library has been loaded.
    ...
    if (configuration_file) { // Checks if the configuration has been sucessfully opened.
      ...
      configuration_json_mqtt["topics"] = configuration.mqtt_topics;
#endif

#if WEBSITE == true
      JsonObject configuration_json_website = configuration_json.createNestedObject("website");
      configuration_json_website["username"] = configuration.website_username;
      configuration_json_website["password"] = configuration.website_password;
#endif
...
// Webserver management
  ...
  //// Gets the HTML file of the homepage. Saves it temporarily to the flash memory. To save RAM.
  // Will be ran, on request processing, for each found variable within a HTML document.
  String website_tag_processor(const String& tag) {
    if (tag == "HOSTNAME") {
      return (configuration.general_hostname);
    } else if (tag == "TIME") {
      return (getTime());
    } else if (tag == "GPIO_OUTPUT") {
      String output;
      for (int i = 0; i < sizeof gpio_outputs / sizeof gpio_outputs[0]; i++) {
        output = output + website_control_switch(gpio_outputs[i], gpio_outputs_names[i]);
      }
      return output;
    } else {
      return String(); // If nothing matches. Return an empty string.
    }
  }
  
  // Renders the HTML code of the on/off switch.
  String website_control_switch(int pin, String pin_name) {
    String switch_value;
    if (digitalRead(pin) == HIGH) {
      String switch_value = "on";
    }
    else {
      String switch_value = "off";
    }
    return "<div class=\"switchcontrol\"><p class=\"switchlabel\">" + String(pin_name) + "</p><label class=\"switch\"><input type=\"checkbox\" value=\"" + switch_value + "\" onchange=\"toggleSwitch(this, " + String(pin) + ")\" id=\"switch_" + String(pin) + "\"><span class=\"slider\"></span></label></div>";
  }
  
  // Renders JSON responses.
  String generateJsonResponse(JsonObject data, String success, String error) {
    DynamicJsonDocument root(512);
    root["success"] = success;
    root["error"] = error;
    root["success"] = getTime();
    JsonObject root_data = root.createNestedObject("data");
  
    for (auto kvp : data)
    {
      root_data[kvp.key()] = kvp.value();
    }
  ...
  // Just an empty JSON document. Helpful in some situations. Like when a response should not return any data.
  StaticJsonDocument<1> empty_doc;
  JsonObject empty_object = empty_doc["empty_document"];
...
void setup(void) {
  ...
  Serial.println("[i] Username: '" + String(configuration.mqtt_username) + "', password: '" + String(configuration.mqtt_password) + "'"); 
  ...
  mqtt_client.subscribe(configuration.mqtt_topics);
  ...
  Serial.println("[*] Configuring the webserver.");

  website.on("/", HTTP_GET, [](AsyncWebServerRequest * request) {
    if (!request->authenticate(configuration.website_username, configuration.website_password))
      return request->requestAuthentication("Super-ESP");
    Serial.println(getTime() + " [i] Website: IPaddress '" + request->client()->remoteIP().toString() + "' has logged in.");
    request->send(SPIFFS, "/homepage.html", String(), false , website_tag_processor);
  });

  website.on("/logout", HTTP_GET, [](AsyncWebServerRequest * request) {
    Serial.println(getTime() + " [i] Website: IPaddress '" + request->client()->remoteIP().toString() + "' has logged out.");
    request->send(401);
  });

  website.on("/logged-out", HTTP_GET, [](AsyncWebServerRequest * request) {
    request->send(SPIFFS, "/logout.html", String(), false , website_tag_processor);
  });

  // Send a GET request to <IP>/api?output=<ouput>&state=<state>
  website.on("/api", HTTP_GET, [] (AsyncWebServerRequest * request) {
    if (!request->authenticate(configuration.website_username, configuration.website_password))
      return request->requestAuthentication("Super-ESP");
    Serial.println(getTime() + " [i] API: IPaddress '" + request->client()->remoteIP().toString() + "' has logged in.");

    // GET input value on <IP>/api?state=<state>
    if (request->hasParam("output")) {
      String output = request->getParam("output")->value();
      if (request->hasParam("state")) {
        String state = request->getParam("state")->value();
        if (isNumeric(output) && isNumeric(state)) {
          if (IsInArray(gpio_outputs, (sizeof(gpio_outputs) / sizeof(gpio_outputs[0])), output.toInt())) {
            int checked_state;
            if (state.toInt() != 1) {
              checked_state = 0;
            }
            ...
            // Log
            Serial.println(getTime() + " [i] API: IPaddress '" + request->client()->remoteIP().toString() + "' changed GPIO '" + String(output) + "' to state '" + String(state) + "'");

            // Do
            digitalWrite(output.toInt(), checked_state);

            // Reply
            StaticJsonDocument<128> doc;
            JsonObject data = doc.to<JsonObject>();
            data["output"] = output;
            data["state"] = String(digitalRead(output.toInt()));
            request->send(200, "application/json", generateJsonResponse(data, "true", "false"));
          }
          else {
            request->send(400, "application/json", generateJsonResponse(empty_object, "false", "GPIO pin has not been registered."));
          }
        }
        else {
          request->send(400, "application/json", generateJsonResponse(empty_object, "false", "Output or state parameters were not given as a number."));
        }
      }
      else {
        if (isNumeric(output)) {
          ...
          request->send(400, "application/json", generateJsonResponse(empty_object, "false", "Output parameter was not given as a number."));
        }
      }
    }
    else {
  ...
  // HTTPS using ESPAsyncWebserver is currently not supported on the ESP32.
  //#if WEBSITE_SSL == true
  //  website.onSslFileRequest([](void * arg, const char *filename, uint8_t **buf) -> int {
  ...
  //  }, NULL);
  //
  //  website.beginSecure("/Cert.pem", "/Key.pem", NULL); // Certificats can ben generated using: openssl genrsa -out Key.pem 1024, and: openssl req -x509 -out Cert.pem -key Key.pem -new -sha256 -subj /CN=your.domain -addext "keyUsage=digitalSignature,keyEncipherment" -addext extendedKeyUsage=serverAuth
  //#else
  website.begin(); // Loads the above configured options and starts the webserver.
  //#endif

  Serial.println("[✓] HTTP server started on port: " + String(WEBSITE_PORT));
